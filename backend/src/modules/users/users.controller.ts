...
export class UsersController {
  ...
  @ApiOperation({
    ...
    description:
      'Returns profile details of the authenticated user including computed is_profile_completed field.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Updates first_name and/or last_name of the authenticated user.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Allows administrators to change the role of any user in the system. Only users with ADMIN role can access this endpoint. Available roles: user, moderator, admin.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Allows administrators to add a new skill with proficiency level to any user. Only users with ADMIN role can access this endpoint. This helps track user capabilities for task assignment.',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({
    status: 400,
    description:
      'Bad Request - Invalid skill data or user already has this skill',
    example: {
      success: false,
      statusCode: 400,
      message: 'Bad request',
  ...
  async addUserSkill(@Param('id') id: string, @Body() dto: AddUserSkillDto) {
  ...
  @ApiOperation({
    ...
    description:
      'Allows administrators to enable or disable any user account. When disabled, the user cannot log in or access the system. Only users with ADMIN role can access this endpoint. Use this for account suspension or reactivation.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Promotes a regular user to moderator role and assigns multiple skills in a single transaction. This endpoint performs multiple operations: 1) Finds the user by email address, 2) Validates the user can be promoted (not already moderator/admin), 3) Updates the user\'s role from "user" to "moderator" in the users table, 4) Batch inserts the provided skills into the user_skills table. Only administrators can access this endpoint. The user must exist and be active to be promoted.',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({
    status: 400,
    description:
      'Bad Request - Invalid email format, user already is moderator/admin, or invalid skills data',
    example: {
      success: false,
      statusCode: 400,
  ...
  async promoteToModerator(@Body() dto: AddModeratorDto) {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves a complete list of all users with moderator role along with their associated skills. This endpoint performs a JOIN query between the users and user_skills tables to provide comprehensive moderator information including their skill sets and proficiency levels. Useful for admin dashboards and moderator management interfaces.',
  })
  @ApiResponse({
    status: 200,
