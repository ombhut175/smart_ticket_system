...
export class TicketsController {
  ...
  @ApiOperation({
    summary: 'Get all tickets with pagination (moderator/admin only)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description:
      'Forbidden - only moderators and admins can access all tickets',
  })
  async findAllForModerators(
    @Request() req,
    @Query() query: TicketQueryDto,
  ): Promise<StandardApiResponse<Ticket[]>> {
    const result = await this.ticketsService.findAllForModerator(
      req.user.role,
      query,
    );
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - cannot access this ticket',
  })
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - only moderators can update tickets',
  })
  @ApiResponse({ status: 404, description: 'Ticket not found' })
  async update(
    @Param('id') id: string,
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - only admins and moderators can delete tickets',
  })
  @ApiResponse({ status: 404, description: 'Ticket not found' })
  async remove(
    @Param('id') id: string,
